    ...
    if (company) {
      ...
      const fetchData = () => {
        Promise.all([
          ...
          localStorage.setItem("dashboardData", JSON.stringify(dashboardRes.data));
  
          setNotifications(notificationsRes.data);
          localStorage.setItem("notifications", JSON.stringify(notificationsRes.data));
  
          setDailyCheckIns(checkinsRes.data);
          localStorage.setItem("dailyCheckIns", JSON.stringify(checkinsRes.data));
        }).catch(console.error);
      };
  
      fetchData();
      const interval = setInterval(fetchData, 600000); // يحدث البيانات كل 10 دقائق
